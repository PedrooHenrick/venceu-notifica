...
const schema = z.object({
  full_name: z.string().trim().min(1, "Nome obrigatorio").max(120),
...
export default function Employees() {
  ...
  const remove = async (id: string) => {
    if (!confirm("Excluir este funcionario? Documentos vinculados tambem serao removidos.")) return;
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border pb-3">
        <div>
          <h1 className="text-xl font-semibold tracking-tight">Funcionarios</h1>
          <p className="text-xs text-muted-foreground">Clique em um funcionario para gerenciar seus documentos.</p>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Select value={companyFilter} onValueChange={setCompanyFilter}>
          <SelectTrigger className="h-8 w-full sm:w-[200px] text-sm"><SelectValue /></SelectTrigger>
          <SelectContent>
      ...
      {companies.length === 0 ? (
        <div className="rounded border border-dashed border-border bg-card p-8 text-center text-sm">
          ...
          <p className="text-xs text-muted-foreground">Funcionarios precisam estar vinculados a uma empresa.</p>
        </div>
      ) : filtered.length === 0 ? (
            ...
            {filtered.map((e) => {
              ...
              return (
                <Link
                  ...
                  className="flex items-center justify-between gap-3 px-3 py-3 hover:bg-secondary/40"
                >
                  <div className="flex min-w-0 items-center gap-3">
                    <span className={cn("flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-[11px] font-semibold text-white", avatarColor(e.full_name))}>
                      {initials(e.full_name)}
                    </span>
                    <div className="min-w-0">
                      <p className="truncate text-sm font-medium text-foreground">{e.full_name}</p>
                      <p className="truncate text-xs text-muted-foreground">
                        {e.role || "—"} {e.companies?.name ? `· ${e.companies.name}` : ""}
                      </p>
                      {a.total > 0 && (
                  ...
                  <div className="flex shrink-0 items-center gap-1">
                    <Button
                      ...
                      variant="ghost"
                      className="h-7 w-7"
                      onClick={(ev) => { ev.preventDefault(); openEdit(e); }}
                    >
                    ...
                    <Button
                      ...
                      variant="ghost"
                      className="h-7 w-7"
                      onClick={(ev) => { ev.preventDefault(); remove(e.id); }}
                    >
                      <Trash2 className="h-3.5 w-3.5 text-status-expired" />
                    </Button>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>{editing ? "Editar funcionario" : "Novo funcionario"}</DialogTitle></DialogHeader>
          <div className="space-y-4">
            <div><Label>Nome completo *</Label><Input value={form.full_name} onChange={(e) => setForm({ ...form, full_name: e.target.value })} /></div>
            <div className="grid gap-4 sm:grid-cols-2">
              <div><Label>CPF</Label><Input value={form.cpf} onChange={(e) => setForm({ ...form, cpf: e.target.value })} /></div>
              <div><Label>Cargo / funcao</Label><Input value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })} /></div>
            </div>
            <div>
              <Label>Empresa *</Label>
